...
from astrid.gui.editor.FileManager import FileManager
from astrid.gui.editor.PyEditor import PyEditorWidget
from astrid.gui.editor.SearchItem import SearchItem
from astrid.gui.editor.SearchWorker import SearchWorker
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.window_font = QFont(
            "Fire Code"
        )  # font needs to be installed in your computer if its not use something else
    ...
    def get_editor(self, path=Path("."), is_python_file=True) -> QsciScintilla:
        editor = PyEditorWidget(self, path=path, is_python_file=is_python_file)
        return editor
    ...
    def set_up_body(self):
        ...
        # setup labels
        folder_label = self.get_side_bar_label(str(self.STATIC_PATH / "icon/folder-icon-blue.svg"), "folder-icon")
        side_bar_layout.addWidget(folder_label)

        search_label = self.get_side_bar_label(str(self.STATIC_PATH / "icon/search-icon.svg"), "search-icon")
        ...
        search_input.textChanged.connect(
            lambda text: self.search_worker.update(
                text, self.file_manager.model.rootDirectory().absolutePath(), self.search_checkbox.isChecked()
            )
        )
    ...
    def search_list_view_clicked(self, item: SearchItem):
        ...
        editor: PyEditorWidget = self.tab_view.currentWidget()
    ...
    def show_dialog(self, title, msg) -> int:
        ...
        dialog.setWindowIcon(QIcon(str(self.STATIC_PATH / "icon/close-icon.svg")))
        dialog.setText(msg)
